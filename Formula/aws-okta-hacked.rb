class AwsOktaHacked < Formula
  desc "Authenticate with AWS using your Okta credentials (hacked)"
  ...
  head "https://github.com/segmentio/aws-okta", :using => :git
  ...
  option "with-hack-version-string", "hack version string"

  def install
    version_str = version
    if build.with? "hack-version-string"
      version_str = "#{version}_hacked"
    end
    system "go", "build", *std_go_args, "-ldflags", "-s -w -X main.Version=#{version_str}"
  ...
  test do
    ...
    PTY.spawn("#{bin}/aws-okta-hacked --backend file add") do |input, output, _pid|
